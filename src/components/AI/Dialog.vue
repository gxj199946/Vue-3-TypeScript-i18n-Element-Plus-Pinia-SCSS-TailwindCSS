...
const where_item = ref("");
function onSelect(item: string): void {
  // selectionMade.value = true;
  startChat(item);
}

// 定义一个方法来移除[[...]]及其内容,移出Ai返回标注类似[1]
const removeSquareBracketsContent = (str: string): string => {
  const regex = /\[\[.*?\]\]/g;
...
async function startChat(question: string) {
  ...
  if (!props.endpoint) {
    console.error("Props endpoint is not provided");
    return;
  }
  //获取当前语言
  //根据当前站点的语言，回答指定的语言答案
  interface Course_filter {
    institution: string;
    major: string;
    course: string;
    type: string;
  ...
  const askai_course_filter = ref<Course_filter>({
    institution: "", //institution这个字段他们还没改，现在还是school所有检索不起作用
    major: "",
    course: "",
    type: "",
    chapter: "",
  ...
  if (typeof localStorage !== "undefined") {
    const askai_course = JSON.parse(localStorage.getItem("askai_course")!);
    //如果是在学习页面
    if (!import.meta.env.SSR) {
      // console.log(window.location);
      if (
        window.location.pathname == "/course/course-item/learning/" ||
        window.location.pathname == "/zh/course/course-item/learning/"
      ) {
        askai_course_filter.value = askai_course as Course_filter;
        console.log(askai_course, "<<<<<<<<<AI回答知识检索筛选条件>>>>>>");
        if (askai_course_filter.value) {
          where_item.value = `(metadata.institution LIKE '%${askai_course_filter.value.institution}%') AND (metadata.major LIKE '%${askai_course_filter.value.major}%') AND  (metadata.course LIKE '%${askai_course_filter.value.course}%') AND (metadata.chapter LIKE '%${askai_course_filter.value.chapter}%') AND (metadata.type LIKE '%${askai_course_filter.value.type}%')`;
          // where_item.value= `(metadata.school LIKE '%${askai_course_filter.value.institution}%') AND (metadata.major LIKE '%${askai_course_filter.value.major}%') AND  (metadata.course LIKE '%${askai_course_filter.value.course}%') AND (metadata.chapter LIKE '%${askai_course_filter.value.chapter}%') AND (metadata.type LIKE '%${askai_course_filter.value.type}%')`
        }
      }
    }
  ...
  nextTick(() => {
    scrollToBottom();
    scrollToAnswer(assistantId);
  });
  //发起请求AI回答问题
  try {
    const response = new AbortController();
    fetchEventSource(props.endpoint, {
      method: "POST",
      headers: {
      ...
      onmessage(ev) {
        req.value = JSON.parse(ev.data).data;
        if (req.value?.type == "chunk") {
          const assistantIndex = questions.value.findIndex(
            (q) => q.role === "assistant" && q.id === assistantId
          );
          const content = req.value?.text;
          loading.value = false;
                ...
                <ul class="question-anwser-section">
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        :id="q.role === 'assistant' ? `answer-${q.id}` : ''"
